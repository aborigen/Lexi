...
export default function WordConnectPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-white pb-10">
      <div className="max-w-7xl mx-auto px-4 py-4 lg:py-8">
        <header className="flex flex-col md:flex-row justify-between items-center mb-6 md:mb-10 space-y-4 md:space-y-0">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-primary/20 rounded-xl border border-primary/30">
              <Gamepad2 className="w-5 h-5 md:w-7 md:h-7 text-primary" />
            </div>
            <div>
              <h1 className="text-xl md:text-3xl font-black tracking-tighter italic uppercase leading-none">LEXI<span className="text-primary">.AI</span></h1>
              <p className="text-[7px] md:text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Word Puzzle</p>
            </div>
          </div>

          <div className="flex flex-wrap justify-center gap-2 md:gap-4 items-center">
            <div className="flex gap-1 md:gap-2">
              <Button variant="ghost" size="icon" onClick={toggleLang} className="rounded-full hover:bg-white/10 h-9 w-9 md:h-10 md:w-10">
            ...
            <div className="glass px-2.5 py-1.5 md:px-5 md:py-2.5 rounded-lg md:rounded-xl flex items-center space-x-2">
              <Trophy className="w-3.5 h-3.5 md:w-4 md:h-4 text-secondary" />
              <div>
                <p className="text-[7px] md:text-[9px] font-bold text-muted-foreground uppercase tracking-wider leading-none mb-0.5">{t('high_score', lang)}</p>
                <p className="text-xs md:text-lg font-black">{highScore.toLocaleString()}</p>
              </div>
            </div>
            
            <div className="glass px-2.5 py-1.5 md:px-5 md:py-2.5 rounded-lg md:rounded-xl bg-primary/5 border-primary/20 flex items-center">
              <div>
                <p className="text-[7px] md:text-[9px] font-bold text-primary uppercase tracking-wider leading-none mb-0.5">{t('score', lang)}</p>
                <p className="text-sm md:text-xl font-black">{score.toLocaleString()}</p>
              </div>
            </div>
            
            <Button variant="outline" size="icon" onClick={handleReset} className="rounded-lg md:rounded-xl h-9 w-9 md:h-10 md:w-10 border-white/10 hover:bg-white/5">
              <RefreshCcw className="w-4 h-4 md:w-5 md:h-5" />
            </Button>
          </div>
        </header>

        <div className="flex flex-col lg:grid lg:grid-cols-[1fr_2fr_1fr] gap-4 md:gap-8 items-stretch">
          <aside className="order-2 lg:order-1">
            <div className="glass p-4 md:p-6 rounded-2xl md:rounded-3xl">
               <h3 className="text-[10px] md:text-xs font-bold text-muted-foreground mb-3 uppercase tracking-widest">{t('found_words', lang)}</h3>
               <div className="flex flex-wrap gap-1.5 md:gap-2">
                 {gameState.foundWords.length > 0 ? (
                   gameState.foundWords.map(word => (
                     <div key={word} className="bg-primary/10 text-primary px-2.5 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest border border-primary/20">
                       {word}
                     </div>
                   ))
                 ) : (
          ...
          <aside className="order-3 lg:order-3 space-y-4 md:space-y-6">
            <AIAdvisor 
              onSuggestionReceived={() => {}}
              gameState={gameState}
              lang={lang}
            />
            <div className="glass p-4 md:p-6 rounded-2xl md:rounded-3xl border-secondary/20 bg-secondary/5">
